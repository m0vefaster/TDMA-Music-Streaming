...
void sendToClientI32(int clntSocket, int32_t *ch, int len,char *type){
...
void sendToClientI64(int clntSocket, int64_t *ch, int len,char *type){
...
void HandleTCPClient(int clntSocket, unsigned char *buffer,int len, int sampleRate) {
	...
	int32_t fixed_quanta[num_hosts*sizeof(int32_t)];
	
	int64_t start_time =0, end_time;
	int64_t time_slot =  (1000000000 / sampleRate); 

	for(i=0;i<num_hosts;i++)
		host_ids[i] = i;

	memset(fixed_quanta,0, num_hosts*sizeof(int32_t));
	...
	sendToClientI32(clntSocket, fixed_quanta, sizeof(int32_t) * num_hosts, "receive_initial data");	
	receiveOkay(clntSocket, "receive_initial_data");

	for(i=0;i<len;i++){
		memset(fixed_quanta,0, num_hosts*sizeof(int32_t));
		fixed_quanta[buffer[i]] = 1;
		end_time = start_time + time_slot;

	 	sendToClientI64(clntSocket, &start_time, sizeof(int64_t), "get_single_timeslot");
	 	sendToClientI64(clntSocket, &end_time, sizeof(int64_t), "get_single_timeslot");
		sendToClientI32(clntSocket, fixed_quanta, sizeof(int32_t) * num_hosts, "receive_initial data");	
